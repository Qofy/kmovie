...
export default function StarRating ({
  ...
  return (<div style={containerStyle} className={className}>
    ...
    <p style={textStyle}>{messages.length === maxRating ? messages[tempRating ? tempRating - 1 : rate - 1] : tempRating || rate || ""}</p>
  </div>
  )
}


function Star({onRating, full, onHoverIn, onHoverOut, size, color}){
  const starStyle = {
    width: `${size}px`,
    height: `${size}px`,
    display: "block",
